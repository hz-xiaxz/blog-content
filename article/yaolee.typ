...
#figure(
  image("/content/article/assets/yaolee/decohoney.svg", height: 20%),
...
== Majorana representation of Yao-Lee model
#definition[
  The $sigma$ and $tau$ degrees of freedom can be written in 6 Majorana modes as
  $
    sigma^alpha tau^beta = i c^alpha c^beta, quad sigma^alpha = - i epsilon^(alpha beta gamma)/2 c^beta d^gamma, quad tau^alpha = - i epsilon^(alpha beta gamma)/2 d^beta d^gamma
  $
]
It actually very easy to check by hand.
